...
namespace FCG.TechChallenge.Jogos.Infrastructure.Outbox
{
    public sealed class OutboxRepository
    {
        ...
        {
            const string sql = @"SELECT id, type, payload::text AS payload, created_utc, processed_utc, error, status FROM public.outboxmessages WHERE status = 0 ORDER BY created_utc LIMIT @take;";
        ...
        {
            const string sql = @"UPDATE public.outboxmessages SET status = 1, processed_utc = now() AT TIME ZONE 'UTC', error = NULL WHERE id = @id;";
            await using var conn = new NpgsqlConnection(_cs);
        ...
        {
            const string sql = @"UPDATE public.outboxmessages SET status = 2, error = @error WHERE id = @id;";
            await using var conn = new NpgsqlConnection(_cs);
